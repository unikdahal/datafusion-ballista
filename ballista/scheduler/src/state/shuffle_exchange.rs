...
use ballista_core::shuffle::{
    ShuffleExchangeEpoch, ShuffleExchangeId, ShuffleExchangeLifecycle,
    ShuffleExchangeSequence,
};
...
impl PublishedShuffleArtifact {
    /// Returns the event sequence that made this artifact visible.
...
pub(crate) enum ShufflePublicationResult {
    ...
    /// event and therefore no event sequence.
    Committed {
    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::CursorAhead { after, current } => write!(
                f,
                "shuffle cursor {} is ahead of current sequence {}",
                after.get(),
                current.get()
            ),
...
pub(crate) struct ShuffleExchangeState {
    ...
    // Output partition -> append-ordered (event sequence, artifact key).
    partition_index:
...
impl ShuffleExchangeState {
    ...
    pub(crate) fn new(id: ShuffleExchangeId) -> Self {
        Self {
            id,
            epoch: ShuffleExchangeEpoch::INITIAL,
    ...
    pub(crate) fn seal(
        &mut self,
        epoch: ShuffleExchangeEpoch,
    ) -> Result<Option<ShuffleExchangeSequence>> {
        self.ensure_epoch(epoch)?;
        if self.lifecycle.is_sealed() {
            return Ok(None);
        }
    ...
    pub(crate) fn artifacts_after(
        ...
        let first_new = entries.partition_point(|(sequence, _)| *sequence <= after);
...
mod tests {
    ...
    fn task_publication_is_atomic_and_partition_indexed() {
        ...
        assert_eq!(
            ...
            ShufflePublicationResult::Committed {
                event_sequence: Some(ShuffleExchangeSequence::new(1))
            }
        );
        ...
        assert!(partition_zero.iter().all(|artifact| artifact.sequence().get() == 1));
        ...
        let partition_one = state
            .artifacts_after(epoch, 1, ShuffleExchangeSequence::INITIAL)
            .unwrap();
    ...
    fn exact_replay_is_idempotent_before_and_after_seal() {
        ...
        assert_eq!(state.seal(epoch).unwrap().map(|s| s.get()), Some(3));
    ...
    fn partition_reads_return_only_events_after_cursor() {
        ...
        let delta = state
            .artifacts_after(epoch, 0, ShuffleExchangeSequence::new(1))
            .unwrap();
        assert_eq!(delta.len(), 1);
        assert_eq!(delta[0].sequence().get(), 2);
        assert_eq!(delta[0].location().map_partition_id, 1);

        assert!(state
            .artifacts_after(epoch, 1, ShuffleExchangeSequence::new(2))
            .unwrap()
            .is_empty());

        assert_eq!(
            state
                .artifacts_after(epoch, 0, ShuffleExchangeSequence::new(3))
                .unwrap_err(),
    ...
    #[test]
    fn seal_is_idempotent_and_sequence_exhaustion_fails_closed() {
        let mut state = exchange();
        let epoch = state.epoch();

        assert_eq!(state.seal(epoch).unwrap().map(|s| s.get()), Some(1));
        ...
        assert!(!exhausted.has_committed_artifacts());
    }
}
