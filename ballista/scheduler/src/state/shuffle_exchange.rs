...
/// Exchange-local identity of one physical shuffle data file.
///
/// Passthrough/range files are keyed by output partition plus file id. Sort
/// shuffle writes one consolidated file per file id, so its output partition is
/// deliberately omitted. This mirrors create_shuffle_path exactly and prevents
/// two producer tasks from claiming the same physical path while still allowing
/// one sort task to expose several logical partition slices from its one file.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
pub(crate) enum ShuffleFileKey {
    /// Passthrough/range path: output partition plus optional file id.
    Passthrough {
        output_partition_id: usize,
        file_id: Option<u64>,
    },
    /// Sort path: one consolidated file per required file id.
    Sort {
        file_id: u64,
    },
}
...
impl ShuffleFileKey {
    fn from_location(location: &PartitionLocation) -> Option<Self> {
        if location.is_sort_shuffle {
            location.file_id.map(|file_id| Self::Sort { file_id })
        } else {
...
impl ShuffleExchangeState {
    ...
    pub(crate) fn new(id: ShuffleExchangeId, output_partition_count: usize) -> Self {
        Self {
...
mod tests {
    ...
    fn exchange() -> ShuffleExchangeState {
        ShuffleExchangeState::new(exchange_id(), OUTPUT_PARTITIONS)
    }
    ...
    fn sort_file_ownership_collision_across_tasks_fails_before_mutation() {
        ...
        assert_eq!(
            ...
            ShuffleExchangeError::ConflictingFileOwnership {
                file: ShuffleFileKey::Sort { file_id: 10 },
    ...
    fn passthrough_file_ownership_matches_partitioned_path_shape() {
        ...
        assert_eq!(
            state
                ...
                .unwrap_err(),
            ShuffleExchangeError::ConflictingFileOwnership {
                file: ShuffleFileKey::Passthrough {
                    output_partition_id: 0,
                    file_id: Some(10),
                },
                existing_producer_task_id: 0,
